...
# testing with chunks
extract_rgb_parallel <- function(site_id, mask_type, exif_dir, photo_exif_data, timefilt = "1200", chunk_size = 200, parallel = TRUE) {
  library(readr)
  library(dplyr)
  library(purrr)
  library(fs)
  ...
  message("Extracting ROI metrics in chunks...")

  handlers("txtprogressbar")
  progressr::with_progress({
    p <- progressr::progressor(steps = ceiling(length(photo_paths) / chunk_size))

    chunks <- split(photo_paths, ceiling(seq_along(photo_paths) / chunk_size))

    if (parallel) future::plan(future::multisession, workers = future::availableCores() - 1)
    on.exit(future::plan(future::sequential), add = TRUE)

    metric_list <- map(chunks, function(chunk) {
      p()
      map_dfr(chunk, function(path) {
        tryCatch({
    ...
    metric_list <- map(chunks, function(chunk) {
      p()
      if (parallel) {
        furrr::future_map_dfr(chunk, extract_metrics, .options = furrr::furrr_options(seed = TRUE))
      } else {
        map_dfr(chunk, extract_metrics)
      }
    })
